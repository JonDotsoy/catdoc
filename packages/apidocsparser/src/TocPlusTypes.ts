...
export namespace TocPlus {
  ...
  export class Item implements Common<toc.Item>{
    ...
    constructor(readonly keyToc: string, readonly _item: toc.Item, readonly uri: string) {
      this['content-type'] = mimeTypes.lookup(uri) || undefined;
      this['charset'] = this['content-type'] ? mimeTypes.charset(this['content-type']) || undefined : undefined;
    }
    ...
    bodyJson(replacer?: ((this: any, key: string, value: any) => any) | undefined, space?: string | number | undefined) {
    ...
    async readBuffer() {
      ...
      this.#body = await this.resolveRefs(this.updateRefProp(this.parseBody(buffer)));
    }

    private updateRefProp(body: any) {
      const intent = (obj: any, prop?: string, descriptor?: TypedPropertyDescriptor<any> & PropertyDescriptor, updateValue?: (newVal: any) => void) => {
    ...
    private parseBody(buffer: Buffer) {
      ...
      if (this['content-type'] === 'application/json') return JSON.parse(buffer.toString(getEncoding()));
      if (this['content-type'] === 'text/yaml') return YAML.parse(buffer.toString(getEncoding()));
      return buffer;
    }
